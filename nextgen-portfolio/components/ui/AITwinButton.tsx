...
import { BotMessageSquare, XCircle, SendHorizontal, Zap, UserCircle, Brain } from "lucide-react";
...
export function AITwinButton() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Hey! I'm Krishna's AI assistant. Ask me anything about his skills, projects, or experience!",
      timestamp: new Date(),
    },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.reply) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: "Sorry, I couldn't process that. Please try again.", timestamp: new Date() },
        ]);
      }
    } catch (error) {
      setIsTyping(false);
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Oops! Something went wrong. Please try again.", timestamp: new Date() },
      ]);
    } finally {
      setIsLoading(false);
    }
      ...
      >
        <div className="relative">
          ...
          <div className="relative flex items-center gap-3 px-6 py-3.5 bg-slate-900 dark:bg-white rounded-full shadow-xl group-hover:shadow-2xl transition-all duration-300">
            ...
            <span className="font-semibold text-sm text-white dark:text-slate-900 hidden sm:block">AI Assistant</span>
                      ...
                      <div 
                        ...
                        dangerouslySetInnerHTML={{
                          __html: msg.content
                            ...
                            .replace(/\*(.*?)\*/g, '<em>$1</em>')
                            .replace(/•/g, '<span class="inline-block w-1.5 h-1.5 bg-current rounded-full mx-2 align-middle"></span>')
                        }}
                      />
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 px-4 py-3 rounded-2xl rounded-tl-md border border-slate-200 dark:border-slate-700">
                    {isTyping ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400 animate-pulse">Typing...</span>
                      </div>
                    ) : (
                      <div className="flex gap-1.5">
                        <div className="w-2 h-2 bg-blue-500 rounded-full animate-bounce" />
